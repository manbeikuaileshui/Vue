<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="addres in address" :key="addres.id">
        <span class="username" :class="{selected: addres.isDefault == 1}">{{addres.consignee}}</span>
        <p @click="changeDefault(addres, address)">
          <span class="s1">{{addres.fullAddress}}</span>
          <span class="s2">{{addres.phoneNum}}</span>
          <span class="s3" v-show="addres.isDefault == 1">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    methods: {
      ...
      async submitOrder() {
        ...
        let data = {
          ...
          consigneeTel: this.userDefaultAddress.phomeNum,  // 手机号

        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <!-- 排序结构 -->
              <ul class="sui-nav">
                <li :class="{ active: isNumber }" @click="changeOrder(1)">
                  <a>
                    综合
                    <span
                      v-show="isNumber"
                      ...
                      :class="{
                        ...
                        'icon-angle-double-up': !isDesc,
                      }"
                    ></span
                  ></a>
                </li>
                <li :class="{ active: !isNumber }" @click="changeOrder(2)">
                  <a>
                    价格
                    <span
                      v-show="!isNumber"
...
export default {
  ...
  beforeMount() {
    // 在发请求之前，把接口需要传递的参数进行整理（在给服务器发送请求之前，把参数整理好，服务器就会返回查询的数据）
    //  复杂写法
  ...
  methods: {
    ...
    getDate() {
      ...
      // 每一次请求完毕,应该把相应的1、2、3级分类的id置空，让它接收下一次相应的id
      // 但还是会把相应的字段带个服务器，置为undefined则不会带给服务器
    ...
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置为undefined
      this.searchParams.categoryName = undefined;
      // 再次发送请求
      this.getDate();
      // 地址栏需要更改：进行路由跳转（现在的路由跳转只是跳转到自己这里）
      // 这里只是删除query参数，如果路径当中出现params参数则不应该删除，路由跳转的时候应该带上
    ...
    removeKeyword() {
      ...
      this.$bus.$emit("clear");
      // 地址栏需要更改：进行路由跳转（现在的路由跳转只是跳转到自己这里）
      // 这里只是删除params参数，如果路径当中出现query参数则不应该删除，路由跳转的时候应该带上
    ...
    attrInfo(attrs, attrValue) {
      // console.log('父组件收到子组件传递过来的参数',attrs, attrValue);
      // 整理参数格式
      let props = `${attrs.attrId}:${attrValue}:${attrs.attrName}`;
      // 数组去重
      // 判断props数组中是否有props元素，没有就添加，有就不添加

<template>
  <div>
    ...
    <el-table style="width: 100%" border :data="list">
      ...
      >
        <template slot-scope="{ row }">
          <el-button type="warning" icon="el-icon-edit" size="mini" @click="undateTradeMark(row)">修改</el-button>
    ...
    <el-dialog :title="tmForm.id ? '修改品牌' : '添加品牌'" :visible.sync="dialogFormVisible">
      <!--
        form表单
        model属性的作用是：把表单的数据收集到哪个对象的身上，将来表单验证也需要这个属性
      -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        // 品牌名称的验证规则
        // required: true 必须要校验字段（前面五角星有关系）
        // message 提示信息
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      // 获取品牌列表的接口
      // 当你向服务器发送请求的时候，这个函数需要带参数，因此在data中初始化两个字段，代表给服务器传递参数
